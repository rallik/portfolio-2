...
const Nav = () => {
    ...
    return (
        <React.Fragment>
            <i className={clicked ? 'nav-icon active' : 'nav-icon'} role='button' aria-clicked={clicked ? 'true' : 'false'} aria-label='nav-icon' onKeyDown={openCloseNav} onClick={openCloseNav}>
                <Menu/>
            </i>
